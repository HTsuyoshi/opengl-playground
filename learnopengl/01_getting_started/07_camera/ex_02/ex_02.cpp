...
int main()
{
	...
	unsigned char *data = stbi_load("../../05_transformations/textures/Wall.jpg", &width, &height, &nrChannels, 0);
	if (data)
	{
	...
	data = stbi_load("../../05_transformations/textures/Apple.png", &width, &height, &nrChannels, 0);
	if (data)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Turning the camera around the object
		...
		glm::mat4 view = calculate_lookAt(
				cameraPos,
		...
		for (unsigned int i = 0; i < 10; ++i)
		{
			...
			model = glm::translate(model, cubePositions[i] + static_cast<float>(glm::sin(glfwGetTime())));
			float angle = 20.0f * i;
